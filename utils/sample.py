...
def csgan_generate_images(z_prior, truncation_factor, batch_size, num_classes,
                          y_sampler, radius, generator, MODEL, device):

    z_content, fake_labels, _ = sample_zy(z_prior=z_prior,
                                          batch_size=batch_size,
                                          z_dim=MODEL.content_dim,
                                          num_classes=num_classes,
                                          truncation_factor=-1,
                                          y_sampler=y_sampler,
                                          radius=radius,
    ...
    z_style, fake_labels, _ = sample_zy(z_prior=z_prior,
                                        batch_size=batch_size,
                                        z_dim=MODEL.style_dim,
                                        num_classes=num_classes,
                                        truncation_factor=-1,
                                        y_sampler=y_sampler,
                                        radius=radius,
    ...
    fake_images = generator(z_content, z_style)

    return fake_images, fake_labels


def csgan_generate_pairwise_images(z_prior, truncation_factor, batch_size, num_classes,
                                   y_sampler, radius, generator, MODEL, device):

    content_a, fake_labels, _ = sample_zy(z_prior=z_prior,
                                          batch_size=batch_size,
                                          z_dim=MODEL.content_dim,
                                          num_classes=num_classes,
                                          truncation_factor=-1,
    ...
    style_a, fake_labels, _ = sample_zy(z_prior=z_prior,
                                        batch_size=batch_size,
                                        z_dim=MODEL.style_dim,
                                        num_classes=num_classes,
                                        truncation_factor=-1,
    ...
    content_b, fake_labels, _ = sample_zy(z_prior=z_prior,
                                          batch_size=batch_size,
                                          z_dim=MODEL.content_dim,
                                          num_classes=num_classes,
                                          truncation_factor=-1,
    ...
    style_b, fake_labels, _ = sample_zy(z_prior=z_prior,
                                        batch_size=batch_size,
                                        z_dim=MODEL.style_dim,
                                        num_classes=num_classes,
                                        truncation_factor=-1,
    ...
    fake_aa = generator(content_a, style_a)
    fake_ab = generator(content_a, style_b)
    fake_ba = generator(content_b, style_a)
    fake_bb = generator(content_b, style_b)

    return fake_aa, fake_ab, fake_ba, fake_bb


def csgan_generate_interpolated_images(z_prior, truncation_factor, batch_size, num_classes,
                                       y_sampler, radius, generator, MODEL, device, num_cols):

    content_start, _, _ = sample_zy(z_prior=z_prior, batch_size=1, z_dim=MODEL.content_dim, num_classes=num_classes,
                                    truncation_factor=-1, y_sampler=y_sampler, radius=radius,
                                    device=device)

    content_end, _, _ = sample_zy(z_prior=z_prior, batch_size=1, z_dim=MODEL.content_dim, num_classes=num_classes,
                                  truncation_factor=-1, y_sampler=y_sampler, radius=radius,
                                  device=device)

    style_start, _, _ = sample_zy(z_prior=z_prior, batch_size=1, z_dim=MODEL.style_dim, num_classes=num_classes,
                                  truncation_factor=-1, y_sampler=y_sampler, radius=radius,
                                  device=device)

    style_end, _, _ = sample_zy(z_prior=z_prior, batch_size=1, z_dim=MODEL.style_dim, num_classes=num_classes,
                                truncation_factor=-1, y_sampler=y_sampler, radius=radius,
                                device=device)

    num_points = num_cols
    content = torch.randn(batch_size, MODEL.content_dim, device=device)
    style = torch.randn(batch_size, MODEL.style_dim, device=device)
    content_delta = (content_end - content_start) / (float(num_points-1))
    style_delta = (style_end - style_start) / (float(num_points-1))

    img_count = 0
    batch_count = 0
    for j in range(num_points):
        for k in range(num_points):
            content[img_count] = content_start + content_delta*float(j)
            style[img_count] = style_start + style_delta*float(k)
            # content[img_count] = content_start*(num_points-j)/(num_points-1.0) + content_end*j/(num_points-1.0)
            # style[img_count] = style_start*(num_points-k)/(num_points-1.0) + style_end*k/(num_points-1.0)
            img_count = img_count + 1
            if img_count == batch_size:
                gen_batch = generator(content, style)
                if batch_count == 0:
                    gen_cat = gen_batch
                else:
                    gen_cat = torch.cat((gen_cat, gen_batch), dim=0)
                img_count = 0
    ...
    return gen_cat[0:num_cols*num_cols]
