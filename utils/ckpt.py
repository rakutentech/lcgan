...
def load_ckpt(model, optimizer, ckpt_path, load_model=False, load_opt=False, load_misc=False, is_freezeD=False):
    ...
    if load_misc:
        ...
        return seed, run_name, step, epoch, best_step, best_fid, best_ckpt_path
...
def load_StudioGAN_ckpts(ckpt_dir, load_best, Gen, Dis, g_optimizer, d_optimizer, run_name, apply_g_ema, Gen_ema, ema,
    ...
    seed, prev_run_name, step, epoch, best_step, best_fid, best_ckpt_path =\
        load_ckpt(model=Dis,
    ...
    if not is_train:
        prev_run_name = cfg_file[cfg_file.rindex("/")+1:cfg_file.index(".yaml")] +\
                        prev_run_name[prev_run_name.index("-train"):]
    ...
    return prev_run_name, step, epoch, best_step, best_fid, best_ckpt_path, logger
